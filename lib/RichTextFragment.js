...
class RichTextFragment {
    ...
    constructor(value, styles, richText) {
        ...
        if (value.name === 'r') {
            this._node = value;
            this._fontNode = xmlq.findChild(this._node, 'rPr') || xmlq.findChild(this._node, 'x:rPr');
            if (!this._fontNode) {
                this._fontNode = { name: 'rPr', attributes: {}, children: [] };
                this._node.children.unshift(this._fontNode);
            }
            this._valueNode = xmlq.findChild(this._node, 't') || xmlq.findChild(this._node, 'x:t');
        } else {
            this._node = {
                name: 'r',
                attributes: {},
    ...
    _getColor(node, name) {
        const child = xmlq.findChild(node, name) || xmlq.findChild(node, `x:${name}`);
