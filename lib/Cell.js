...
class Cell {
    ...
    _parseNode(node) {
        ...
        const fNode = xmlq.findChild(node, 'f') || xmlq.findChild(node, 'x:f');
        if (fNode) {
        ...
        if (type === "s") {
            // String value.
            const vNode = xmlq.findChild(node, 'v') || xmlq.findChild(node, 'x:v');
            if (vNode) {
            ...
            const isNode = xmlq.findChild(node, 'is') || xmlq.findChild(node, 'x:is');
            ...
            // Boolean value.
            this._value = (xmlq.findChild(node, 'v') || xmlq.findChild(node, 'x:v')).children[0] === 1;
        } else if (type === "e") {
            // Error value.
            const error = (xmlq.findChild(node, 'v') || xmlq.findChild(node, 'x:v')).children[0];
